...
export default function JournalPanel({ data, mutate }) {
  ...
  const summary = useMemo(() => {
    ...
    // first / last date in the filtered window
    let first = null;
    let last = null;
    filtered.forEach((e) => {
    ...
    <div>
      ...
      <Card>
        ...
        {filtered.length > 50 && (
          ...
          >
            Showing 50 of {filtered.length} entries. Refine your filter to see
            more.
          </div>
        )}
      </Card>
                ...
                {groups.length > 0 && (
                  <tfoot>
                    ...
                    >
                      ...
                      <Td right mono>
                        GHS{" "}
                        {fmt(groups.reduce((s, g) => s + g.debits, 0))}
                      </Td>
                      <Td right mono>
                        GHS{" "}
                        {fmt(groups.reduce((s, g) => s + g.credits, 0))}
                      </Td>
                      <Td right mono>
                        GHS{" "}
                        {fmt(groups.reduce((s, g) => s + g.total, 0))}
                      </Td>
                    </tr>
                  </tfoot>
                )}
                ...
                {viewingEntry.lines.map((l, i) => {
                  ...
                  return (
                    <tr key={i} className="row-hover">
                      <Td>
                        {acc ? `${acc.code} — ${acc.name}` : l.account}
                      </Td>
                      <Td right mono>
