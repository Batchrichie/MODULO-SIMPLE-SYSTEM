...
export default function LoansPanel({ profile }: { profile: UserProfile }) {
  ...
  async function voidLoan() {
    ...
    if (!reason) {
      setActionError('A reason is required to void a loan.');
      return;
    }
    setSaving(true);
    setActionError('');
    try {
      await callRpc('void_loan', {
        p_loan_id: actionLoan.id,
        p_performed_by: profile.employeeId,
        p_reason: reason,
      });
      setActionLoan(null);
      setVoidReason('');
      ...
      setActionError(err instanceof Error ? err.message : 'Unable to void loan.');
    } finally { setSaving(false); }
  }
  ...
  return <div>
    ...
    {loading ? <div style={{ color: MUTED, padding: 30 }}><Loader2 size={16} /> Loading loans...</div> : error ? <Card><div style={{ color: ALERT }}>Unable to load loan data: {error}</div></Card> : <>
      ...
      {actionLoan && <Card style={{ marginBottom: 18, border: '1px solid var(--green)' }}><h3 style={{ marginTop: 0, color: INK }}>{actionLoan.status === 'Pending' ? 'Process loan request' : actionLoan.status === 'Active' ? 'Void active loan' : 'Loan action'}</h3><div style={{ color: MUTED, fontSize: 13, marginBottom: 12 }}>{actionLoan.employee?.name || actionLoan.employee_id} · GHS {Number(actionLoan.amount || 0).toFixed(2)}</div>{actionLoan.status === 'Pending' && <><div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}><Button size="sm" variant={repaymentType === 'auto' ? 'primary' : 'ghost'} onClick={() => setRepaymentType('auto')}>Automatic repayment</Button><Button size="sm" variant={repaymentType === 'manual' ? 'primary' : 'ghost'} onClick={() => setRepaymentType('manual')}>Manual repayment</Button></div>{repaymentType === 'auto' && <input style={{ ...inputStyle, marginTop: 12 }} type="number" min="0" placeholder="Installment amount" value={installmentAmount} onChange={(event) => setInstallmentAmount(event.target.value)} />}<input style={{ ...inputStyle, marginTop: 12 }} placeholder="Rejection reason (optional)" value={rejectionReason} onChange={(event) => setRejectionReason(event.target.value)} />{repaymentType === 'manual' && <div style={{ color: MUTED, fontSize: 12, marginTop: 10 }}>Repayments can be recorded manually after approval.</div>}</>}{actionLoan.status === 'Active' && <input style={{ ...inputStyle, marginTop: 12 }} placeholder="Void reason (required)" value={voidReason} onChange={(event) => setVoidReason(event.target.value)} />}{actionError && <div style={{ color: ALERT, fontSize: 12, marginTop: 10 }}>{actionError}</div>}<div style={{ display: 'flex', gap: 8, justifyContent: 'flex-end', marginTop: 14 }}><Button variant="ghost" onClick={() => setActionLoan(null)}>Cancel</Button>{actionLoan.status === 'Pending' && <Button disabled={saving} onClick={approveLoan}>{saving ? 'Saving...' : 'Confirm approval'}</Button>}{actionLoan.status === 'Pending' && <Button variant="danger" disabled={saving} onClick={rejectLoan}>{saving ? 'Saving...' : 'Confirm rejection'}</Button>}{actionLoan.status === 'Active' && <Button variant="danger" disabled={saving} onClick={voidLoan}>{saving ? 'Saving...' : 'Confirm void'}</Button>}</div></Card>}
      <Card><h3 style={{ marginTop: 0, color: INK }}>All loans</h3>{loans.length === 0 ? <div style={{ color: MUTED }}>No loans found.</div> : loans.map((loan) => { const paid = repayments.filter((repayment) => repayment.loan_id === loan.id).reduce((sum, repayment) => sum + Number(repayment.amount || 0), 0); const canVoid = loan.status === 'Active' && Number(loan.balance_remaining ?? loan.amount) === Number(loan.amount) && paid === 0; return <div key={loan.id} style={{ display: 'flex', justifyContent: 'space-between', gap: 12, padding: '10px 0', borderTop: '1px solid var(--rule)', flexWrap: 'wrap' }}><span>{loan.employee?.name || loan.employee_id} · <strong style={{ color: loan.status === 'Voided' ? ALERT : loan.status === 'Active' ? GOLD : INK }}>{loan.status}</strong></span><span style={{ fontFamily: FONT_MONO }}>GHS {Number(loan.amount || 0).toFixed(2)} <small style={{ color: GOLD }}>({paid.toFixed(2)} repaid)</small>{canVoid && <Button size="sm" variant="danger" onClick={() => { setActionLoan(loan); setVoidReason(''); setActionError(''); }}>Void</Button>}</span></div>; })}</Card>
    </>}
  </div>;
}
