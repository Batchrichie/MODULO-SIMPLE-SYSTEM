...
export const NAV_CONFIG: NavItemConfig[] = [
  ...
  // ── CEO field activity (admin only, appears in Operations) ──
  { key: "field-activity",     label: "Field Activity",   icon: Radio,           token: ALL,               group: "Operations" },
...
export function getMobileBottomNav(permissions: string[]): NavItemConfig[] {
  ...
  // WD / Employee: projects, media-library, my-payslips, my-statement
  const keys = ["projects", "media-library", "my-payslips", "my-statement"];
  return keys
    .map((k) => accessible.find((n) => n.key === k))
    .filter(Boolean) as NavItemConfig[];
}
